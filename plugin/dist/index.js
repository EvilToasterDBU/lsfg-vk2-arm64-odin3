...
const manifest = {"name":"LSFG-VK 2 ARM64","author":"Kurt Himebauch; Odin 3 ARM64 Hot-1X adaptation","flags":[],"api_version":1,"publish":{"tags":["installer","vulkan","lsfg","framegen","lossless","scaling"],"description":"Self-contained patched lsfg-vk 2 ARM64 for Odin 3 / Armada with live 1X passthrough and 2X-4X frame generation.","image":"https://raw.githubusercontent.com/xXJSONDeruloXx/decky-lsfg-vk/refs/heads/main/assets/Decky_LSFG-VK_Master_1.png"}};
const API_VERSION = 2;
    ...
    return (window.SP_REACT.createElement(window.SP_REACT.Fragment, null,
        ...
        window.SP_REACT.createElement(DFL.PanelSectionRow, null,
            window.SP_REACT.createElement("div", { style: {
                    ...
                    paddingBottom: "3px",
                    color: "white"
                } }, "Legacy workarounds (not applied in clean mode)")),
        window.SP_REACT.createElement(DFL.PanelSectionRow, null,
    ...
    return (window.SP_REACT.createElement(window.SP_REACT.Fragment, null,
        ...
        window.SP_REACT.createElement(DFL.PanelSectionRow, null,
            window.SP_REACT.createElement("div", { style: {
                ...
                window.SP_REACT.createElement("strong", null, "/usr/libexec/armada/armada-game-launch ~/.local/bin/lsfg-vk2-arm64 %command%"))),
        window.SP_REACT.createElement(DFL.PanelSectionRow, null,
            window.SP_REACT.createElement("div", { style: {
                    fontSize: "11px",
                    lineHeight: "1.3",
                    opacity: "0.6",
                    marginTop: "8px"
                } }, "The configuration is stored in ~/.config/decky-lsfg-vk2-arm64/conf.toml and hot-reloads while games are running."))));
}
...
async function copyToClipboard(text) {
    ...
    // Steam UI-native clipboard APIs vary between client builds. Try every
    // known location first, but still run browser fallbacks afterward.
    const steamClient = window.SteamClient;
    const steamCandidates = [
        [steamClient?.System, steamClient?.System?.SetClipboardText],
        [steamClient?.System, steamClient?.System?.CopyTextToClipboard],
        [steamClient?.Clipboard, steamClient?.Clipboard?.SetText],
    ];
    for (const [owner, method] of steamCandidates) {
        if (typeof method !== "function")
            continue;
        try {
            await Promise.resolve(method.call(owner, text));
            copySuccess = true;
        }
        catch (error) {
            console.debug("Steam clipboard API failed, trying fallback", error);
        }
    }
    ...
    if (navigator.clipboard?.writeText) {
        try {
            ...
            console.debug("navigator.clipboard failed, trying execCommand", error);
        }
    }
...
function SmartClipboardButton() {
    ...
    const getLaunchOptionText = async () => {
        try {
            ...
            return result.launch_option || "/usr/libexec/armada/armada-game-launch ~/.local/bin/lsfg-vk2-arm64 %command%";
        }
        catch (error) {
            return "/usr/libexec/armada/armada-game-launch ~/.local/bin/lsfg-vk2-arm64 %command%";
        }
    };
...
function FgmodClipboardButton() {
    ...
    const copyToClipboard = async () => {
        ...
        setIsLoading(true);
        try {
            const text = "~/fgmod/fgmod /usr/libexec/armada/armada-game-launch ~/.local/bin/lsfg-vk2-arm64 %command%";
            const { success, verified } = await copyWithVerification(text);
            if (success) {
    ...
    return (window.SP_REACT.createElement(DFL.PanelSectionRow, null,
        window.SP_REACT.createElement(DFL.Focusable, { style: {
            ...
            window.SP_REACT.createElement("div", { style: {
                    ...
                    minWidth: "60px",
                    textAlign: "center"
                } }, config.multiplier < 2 ? "1X" : `${config.multiplier}X`),
            window.SP_REACT.createElement(DFL.DialogButton, { style: {
                    marginLeft: "0px",
                    height: "30px",
                    display: "flex",
...
var flatpakTargetImage = 'http://127.0.0.1:1337/plugins/LSFG-VK 2 ARM64/assets/flatpak-target-34f0e3b7.png';
...
function Content() {
    ...
    return (window.SP_REACT.createElement(DFL.PanelSection, null,
        ...
        isInstalled && (window.SP_REACT.createElement(window.SP_REACT.Fragment, null,
            ...
            config.multiplier < 2 && window.SP_REACT.createElement("div", { style: { fontSize: "11px", opacity: 0.75, margin: "0 12px 6px" } }, "1X is live native passthrough. Switch between 1X and 2X-4X without restarting the game."))),
...
var index = definePlugin(() => {
    ...
    return {
        name: "LSFG-VK 2 ARM64",
        titleView: window.SP_REACT.createElement("div", { className: DFL.staticClasses.Title }, "LSFG-VK 2 ARM64"),
        alwaysRender: true,
