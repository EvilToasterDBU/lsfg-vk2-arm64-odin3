...
class Plugin:
    ...
    async def _main(self):
        ...
        """
        try:
            status = self.installation_service.check_installation()
            if not status.get("installed"):
                decky.logger.info("Installing or repairing bundled LSFG-VK 2 ARM64 Hot-1X engine")
                install_result = self.installation_service.install()
                if not install_result.get("success"):
                    decky.logger.error("Bundled engine installation failed: %s", install_result.get("error"))
            status = self.installation_service.check_installation()
            if status.get("installed"):
                profile_data = self.configuration_service._get_profile_data()
                result = self.configuration_service.update_lsfg_script_from_profile_data(profile_data)
                if result.get("success"):
                    decky.logger.info("Regenerated LSFG-VK 2 ARM64 Hot-1X wrapper")
                else:
                    decky.logger.warning("Could not regenerate Hot-1X wrapper: %s", result.get("error"))
        except Exception as exc:
            decky.logger.warning("Could not initialize LSFG-VK 2 ARM64: %s", exc)
        decky.logger.info("LSFG-VK 2 ARM64 plugin loaded")
