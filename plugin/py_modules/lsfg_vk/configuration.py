...
# ---------------------------------------------------------------------------
# Isolated lsfg-vk 2 ARM64 wrapper generation
# ---------------------------------------------------------------------------
def _shell_single_quote(value: str) -> str:
    return "'" + str(value).replace("'", "'\\''") + "'"


def _v2_wrapper_lines(service, config, profile_name):
    install_root = service.user_home / ".local/share/decky-lsfg-vk2-arm64"
    manifest_dir = install_root / "layer"
    config_path = service.user_home / ".config/decky-lsfg-vk2-arm64/conf.toml"
    q_manifest = _shell_single_quote(str(manifest_dir))

    return [
        "#!/bin/bash",
        "# LSFG-VK 2 ARM64 Hot-1X launcher generated by Decky.",
        "# Armada launch option: /usr/libexec/armada/armada-game-launch ~/.local/bin/lsfg-vk2-arm64 %command%",
        "unset VK_ADD_IMPLICIT_LAYER_PATH",
        f"export VK_IMPLICIT_LAYER_PATH={q_manifest}",
        f"export LSFGVK_CONFIG={_shell_single_quote(str(config_path))}",
        f"export LSFGVK_PROFILE={_shell_single_quote(profile_name)}",
...
def _v2_generate_script_content(self, config: ConfigurationData) -> str:
    return "\n".join(_v2_wrapper_lines(self, config, DEFAULT_PROFILE_NAME)) + "\n"


def _v2_generate_script_for_profile(self, profile_data: ProfileData) -> str:
    current = profile_data["current_profile"]
    config = dict(profile_data["profiles"].get(current, ConfigurationManager.get_defaults()))
    config.update(profile_data.get("global_config", {}))
    return "\n".join(_v2_wrapper_lines(self, config, current)) + "\n"


ConfigurationService._generate_script_content = _v2_generate_script_content
ConfigurationService._generate_script_content_for_profile = _v2_generate_script_for_profile
