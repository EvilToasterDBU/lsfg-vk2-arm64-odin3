...
# ---------------------------------------------------------------------------
# Odin 3 ARM64 / lsfg-vk 2.0 configuration adapter
# ---------------------------------------------------------------------------
def _v2_toml_escape(value: str) -> str:
    return str(value).replace("\\", "\\\\").replace('"', '\\"')


def _v2_generate_multi(profile_data: ProfileData) -> str:
    lines = [
        "version = 2",
        "",
        "# Selected by the Decky wrapper through LSFGVK_PROFILE.",
        f'# decky_current_profile = "{_v2_toml_escape(profile_data["current_profile"])}"',
        "",
        "[global]",
    ]
    dll_path = str(profile_data.get("global_config", {}).get("dll", "") or "")
    if dll_path:
        lines.append(f'dll = "{_v2_toml_escape(dll_path)}"')
    no_fp16 = bool(profile_data.get("global_config", {}).get("no_fp16", False))
    lines.append(f"allow_fp16 = {'false' if no_fp16 else 'true'}")
    lines.append("")

    profiles = profile_data.get("profiles", {}) or {
        DEFAULT_PROFILE_NAME: ConfigurationManager.get_defaults()
    }
    for name, config in sorted(
        profiles.items(), key=lambda item: (item[0] != DEFAULT_PROFILE_NAME, item[0])
    ):
        requested_multiplier = int(config.get("multiplier", 2) or 2)
        multiplier = max(1, min(4, requested_multiplier))
        flow_scale = max(0.1, min(1.0, float(config.get("flow_scale", 1.0) or 1.0)))
        lines.extend([
            "[[profile]]",
            f'name = "{_v2_toml_escape(name)}"',
            # Patched ARM64 layer supports multiplier=1 as live passthrough.
            f"multiplier = {multiplier}",
            f"flow_scale = {flow_scale:g}",
            f"performance_mode = {str(bool(config.get('performance_mode', False))).lower()}",
            'pacing = "none"',
            "",
        ])
    return "\n".join(lines)
...
def _v2_parse_multi(content: str) -> ProfileData:
    profiles = {}
    ...
    def save_current():
        nonlocal current_name, current_config
        if current_name and current_config is not None:
            profiles[current_name] = cast(ConfigurationData, current_config)
        current_name = None
        current_config = None
    ...
    for original in content.splitlines():
        ...
        if not stripped:
            continue
        if stripped.startswith("# decky_current_profile") and "=" in stripped:
            current_profile = str(_v2_parse_value(stripped.split("=", 1)[1]))
            continue
        ...
        if stripped == "[[profile]]":
            save_current()
            section = "profile"
            current_config = dict(ConfigurationManager.get_defaults())
            current_config["multiplier"] = 2
            current_config["flow_scale"] = 1.0
            current_config["hdr_mode"] = False
            current_config["experimental_present_mode"] = "fifo"
            continue
        if "=" not in stripped:
            continue
        key, raw = (part.strip() for part in stripped.split("=", 1))
        value = _v2_parse_value(raw)
        if section == "global":
            if key == "dll":
                global_config["dll"] = str(value)
            elif key == "allow_fp16":
                global_config["no_fp16"] = not bool(value)
        elif section == "profile" and current_config is not None:
            if key == "name":
                current_name = str(value)
            elif key in {"multiplier", "flow_scale", "performance_mode"}:
...
def _v2_generate_single(config: ConfigurationData) -> str:
    return _v2_generate_multi(ProfileData(
        current_profile=DEFAULT_PROFILE_NAME,
        profiles={DEFAULT_PROFILE_NAME: config},
        global_config={
            "dll": config.get("dll", ""),
            "no_fp16": bool(config.get("no_fp16", False)),
        },
    ))


def _v2_parse_single(content: str) -> ConfigurationData:
    data = _v2_parse_multi(content)
    config = dict(data["profiles"].get(data["current_profile"], ConfigurationManager.get_defaults()))
    config["dll"] = data["global_config"].get("dll", "")
    config["no_fp16"] = data["global_config"].get("no_fp16", False)
    return cast(ConfigurationData, config)


ConfigurationManager.generate_toml_content = staticmethod(_v2_generate_single)
ConfigurationManager.generate_toml_content_multi_profile = staticmethod(_v2_generate_multi)
ConfigurationManager.parse_toml_content = staticmethod(_v2_parse_single)
ConfigurationManager.parse_toml_content_multi_profile = staticmethod(_v2_parse_multi)
