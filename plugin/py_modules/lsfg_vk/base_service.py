...
class BaseService:
    ...
    def __init__(self, logger: Optional[Any] = None):
        ...
        # Path.home() can resolve to Decky/FEX's service home on Armada rather
        # than the interactive user's home. Decky exposes the real target home.
        decky_user_home = getattr(decky, "DECKY_USER_HOME", None)
        env_user_home = os.environ.get("DECKY_USER_HOME")
        self.user_home = Path(decky_user_home or env_user_home or Path.home()).expanduser()
        self.log.info(f"Using Decky user home: {self.user_home}")
